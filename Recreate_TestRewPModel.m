function [ArtificialBurst ConstructedWaveform Model_AIC BurstMaximum] = Recreate_TestRewPModel(params,model,Pun_ERP,Rew_ERP,tx2disp,srate,n,time1,time2,UseBaseline); 
...
if strcmp(model,'Full')
    ...
    ConstructedWaveform = Pun_ERP(n,:) + TIME_Full;
    
    % Compare simulated with Real
    T1 = find(tx2disp(1,:)==time1); T2 = find(tx2disp(1,:)==time2);
    SSE_Full = sum((Rew_ERP(n,T1:T2)-ConstructedWaveform(1,T1:T2)).^2);
    Model_AIC = NumOfSamples .* log(SSE_Full./NumOfSamples) + 2.* NumOfParams;
    
    % Get Burst Maximum for reporting
    if SCALE_fit >=0
        [BurstPeak, BurstPeakLoc] = max(ArtificialBurst);
    elseif SCALE_fit <0
        [BurstPeak, BurstPeakLoc] = min(ArtificialBurst);
    end
    BurstMaximum = tx2disp(BurstPeakLoc);
...
elseif strcmp(model,'Half')
    ...
    ConstructedWaveform = Pun_ERP(n,:) + TIME_Half;
    
    % Compare simulated with Real
    T1 = find(tx2disp(1,:)==time1); T2 = find(tx2disp(1,:)==time2);
    SSE_Half = sum((Rew_ERP(n,T1:T2)-ConstructedWaveform(1,T1:T2)).^2);
...
elseif strcmp(model,'Gauss')
    ...
    ConstructedWaveform = Pun_ERP(n,:) + TIME_Gauss;
    
    % Compare simulated with Real
    T1 = find(tx2disp(1,:)==time1); T2 = find(tx2disp(1,:)==time2);
    SSE_Gaus = sum((Rew_ERP(n,T1:T2)-ConstructedWaveform(1,T1:T2)).^2);
    Model_AIC = NumOfSamples .* log(SSE_Gaus./NumOfSamples) + 2.* NumOfParams;
...
elseif strcmp(model,'Wavelet')
    ...
    ConstructedWaveform = Pun_ERP(n,:) + TIME_Wavelet;
    
    % Compare simulated with Real
    T1 = find(tx2disp(1,:)==time1); T2 = find(tx2disp(1,:)==time2);
    SSE_Wave = sum((Rew_ERP(n,T1:T2)-ConstructedWaveform(1,T1:T2)).^2);
    Model_AIC = NumOfSamples .* log(SSE_Wave./NumOfSamples) + 2.* NumOfParams;
...
elseif strcmp(model,'Gamma')
    ...
    ConstructedWaveform = Pun_ERP(n,:) + TIME_Gamma;
    
    % Compare simulated with Real
    T1 = find(tx2disp(1,:)==time1); T2 = find(tx2disp(1,:)==time2);
    SSE_Gama = sum((Rew_ERP(n,T1:T2)-ConstructedWaveform(1,T1:T2)).^2);
    Model_AIC = NumOfSamples .* log(SSE_Gama./NumOfSamples) + 2.* NumOfParams;
...
elseif strcmp(model,'TwoHalf')
    ...
    % Create Artificial Burst and Constructed Waveform for plotting
    ArtificialBurst = TIME_2Half;
    ConstructedWaveform = Pun_ERP(n,:) + TIME_2Half;
    
    % Compare simulated with Real
    T1 = find(tx2disp(1,:)==time1); T2 = find(tx2disp(1,:)==time2);
    SSE_2Half = sum((Rew_ERP(n,T1:T2)-ConstructedWaveform(1,T1:T2)).^2);
